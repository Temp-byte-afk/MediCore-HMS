...
MarkNoShowScreen::MarkNoShowScreen(MediCoreSystem& backend, ScreenManager& screenManager, const char* fontFilePath)
    : BaseScreen(backend, screenManager),
    m_appointmentIdInput()
{
    m_navigationRequested = false;
    m_requestedState = ScreenState::MarkNoShow;
    ...
    m_titleLabel = Label(m_font, "Mark Appointment No-Show", 34, sf::Vector2f(70.f, 30.f));
    m_hintLabel = Label(m_font, "Only today's pending appointments can be marked.", 20, sf::Vector2f(70.f, 80.f));
    ...
    m_submitButton = Button(m_font, "Mark No-Show", sf::Vector2f(70.f, 325.f), sf::Vector2f(180.f, 50.f));
...
void MarkNoShowScreen::requestScreen(ScreenState state)
{
    m_requestedState = state;
    m_navigationRequested = true;
}

void MarkNoShowScreen::submitNoShow()
{
    ...
    try
    {
        m_backend.markAppointmentNoShow(doctorId, appointmentId);
        m_messagePanel.setMessage("Appointment marked as no-show.");
    }
...
void MarkNoShowScreen::handleEvent(const sf::Event& event, const sf::RenderWindow& window)
{
...
void MarkNoShowScreen::update(sf::Time deltaTime)
{
    (void)deltaTime;
    m_appointmentIdInput.update();
}

void MarkNoShowScreen::render(sf::RenderWindow& window)
{
...
bool MarkNoShowScreen::hasNavigationRequest() const
{
    return m_navigationRequested;
}

ScreenState MarkNoShowScreen::getRequestedState() const
{
    return m_requestedState;
}

void MarkNoShowScreen::clearNavigationRequest()
{
    m_navigationRequested = false;
}